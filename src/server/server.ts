...
/**
 * 密钥的脱敏形态（健康检查回给**未鉴权的任何人**，启动横幅给操作者）。
 *
 * ★ **只留后 4 位，不留前缀。** 前缀是各家公开的标识格式（`vck_` 属于
 * Vercel AI Gateway、`sk-or-v1-` 属于 OpenRouter），把它打出来等于把
 * 「免费试用背后是哪一家」直接说出去 —— 本文件其他地方（错误文案、
 * 健康检查的字段、上游表）刻意藏起来的正是这一条，留个前缀就全抵消了。
 *
 * 后 4 位是随机的，够操作者确认「配的是哪一把」，认不出是谁家的。
 *
 * ⚠ 与 `api/_upstream.ts` 的 `mask` 是同源的重复，改一处记得改另一处。
 */
 ...
 * 正是这个项目反复出现的那个母题**（同 `_upstream.ts` 里「429 与
 * 『模型没答』长得一模一样」那条）。
 *
 * ⚠ 400 与 401/403 这两条分支里**不能说「换个后端」**，也**不许出现
 * 厂商名或端点**：错误信息恰恰是用户最会仔细读的地方，写上「上游是谁」
 * 就等于把前面那些隐藏全抵消了（同 `masked` 踩过的那个坑）。
 * 只描述**用户能观察到的类别**，不描述是谁拒的。
 *
 * ⚠ 与 `api/_upstream.ts` 的 `publicError` 是同源的重复，改一处记得改另一处。
 */
