...
test("出厂节奏是基准的四倍 —— 用户连着两轮都说「还不够慢」", () => {
  // 这条是**规格守卫**，不是实现细节：用户看着实物调了两轮（420 → 840 → 1680），
  // 两次的理由都是「看不清谁在哪儿落的子」。谁要是把它调快，这里会红 ——
  // 那正是需要先回头看一眼规格的时刻
  assert.equal(FLIP_MS, FLIP_MS_BASE * 4, `出厂落子相时长被改快了：${FLIP_MS}ms`);
  assert.ok(FLIP_MS >= 1500, "落子相短于 1.5 秒就回到「看不清」了");
});
