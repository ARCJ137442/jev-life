...
export function templateVars(i: TemplateInput): Record<string, string> {
  ...
  // ★ 单人局里行动方叫**玩家**，与界面侧同一个称呼（`mode.ts` 的 SOLO 表）。
  //
  // 只换「名字」，不动人称：这份规则说明里本来就有一批第二人称的说法
  // （「你获胜」「你翻一格」「你落败」），那些是**直接称呼**而不是角色名，
  // 而且「你」比「玩家」更贴着一对一对话的语感（用户 2026-09-21 定）。
  //
  // `roleDirection(role)` 也照旧 —— 它给的是**方向词**（尽可能多 / 尽可能地少），
  // 那条判据在单人局里仍然成立（你要的是让细胞活下来），换的只是名字。
