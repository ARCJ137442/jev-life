...
/* ── 4×4：退化演示 ──
   ...
   目前只放振荡器：摆动本身就让后继落到新局面里，是这一档最能看清「演化」的形态。

   ⚠ 曾有一版注释说「方块在这一档会撞上 repeatBlocked，第 1 代就走投无路终局」——
   那描述的是**已废弃的按角色判定语义**（该实现只看「单个角色走一步 + 演化」，
   忽略了同回合另一方的落子）。现行语义下 repeatBlocked 是**回合级**的：
   只有当双方所有落点组合都推不动局面时才成立。实测 4×4 上「单个方块」
   「两个方块（对角）」「信号灯」三种情形**双方都是 null** —— 方块自己很安静，
   但生之执往它旁边落子就能改变局面，游戏推得动。 */
...
const OPENINGS_8: readonly Opening[] = [
  ...
  opening({
    ...
    note:
      ...
      "⚠ 单个方块更极端：敲掉它任一角都会在一代后长回原样，死之执**一个能改变局面的落点都没有**。" +
      "但游戏并不因此结束 —— 生之执往旁边落子就能推动局面（repeatBlocked 是回合级的，不是按角色判的）；" +
...
/**
 * 纯方块阵列：方块之间只隔 1 格，一行就是 `##.##.##.##.`。
 *
 * 这是「**最大化存活格子**」这条轴最纯粹的形态 —— 满密度的静物铺满内区，
 * 16 个方块 / 64 格 / 25.0%，且**完全静止**（实测曲线 64 → 64 → 64 …）。
 *
 * ⚠ 一个曾经被写错、已实测推翻的说法：这里**不需要**掺振荡器来避免死局。
 * 曾有一版注释断言「纯方块阵列是死局，死之执的每一个落点都会一代后长回原局面，
 * 第 1 代就 repeatBlocked 终局」——**实测不成立**：死之执 64 个落点里只有
 * **4 个**（6%）会回到原局面，其余 60 个都会改变局面。
 * 原因是方块之间那条 1 格空隙：敲掉一格后，空隙格凑够 3 个邻居一起诞生，
 * 相邻两块并成一条 5 格横条。所以它不是死局，扰动会真的改变局面。
 *
 * 间距取 3（2 格方块 + 1 格空隙）是「两个方块互不干扰」的最小值：再紧就重叠了。
...
function blockMesh(cols: number, rows: number): string[] {
  ...
  // 铺满内区：行/列都从 PAD 起、步长 3（2 格方块 + 1 格空隙）
  for (let r = PAD; r + 1 < rows - PAD; r += 3) {
    for (let c = PAD; c + 1 < cols - PAD; c += 3) put("block", r, c);
  }
...
const OPENINGS_16: readonly Opening[] = [
  opening({
    ...
    build: blockMesh,
    note:
      "纯方块阵列：方块之间只隔 1 格，一行是 ##.##.##.##.，铺满内区。" +
      "16 个方块 / 64 格 / 25.0%，是「最大化存活格子」这条轴最纯粹的形态。" +
      "⚠ 棋盘**完全静止**（实测十代 64 → 64 → 64 … 不变），但别被「静止」骗了：" +
      "死之执 64 个落点里只有 4 个会回到原局面，其余 60 个都会改变局面 —— " +
      "敲掉一格之后，方块之间那条 1 格空隙会凑够 3 个邻居一起诞生，相邻两块并成一条 5 格横条。",
  }),
  opening({
    id: "dense-random",
    nameZh: "高密度随机",
...
const RULES_8: GameRules = {
  turnLimit: 90,
  // ★ 0.6 → 0.30 → 0.33（2026-09-21，用户试玩后连调两次）
  //
  // 0.6 从来没被标定过 —— 它是设计文档里的示例数字。而它实际意味着生之执要
  // 把活细胞从开局的 9 格（14.1%）堆到 **38 格**（净增 29），几乎是「把大半个
  // 棋盘变成活细胞」；而死之执那边只要把局面压到 3 格（5%）就赢。
  // **两边的难度差得太远**，这不是一条对称的胜负线。
  //
  // 0.30（19 格，需净增 10）是用户选的新起点；0.33（22 格）是单人局实测之后
  // 微调的。判据要的是**连续** `lifeStreak` 回合**每一代**都不跌破这条线，
  // 而「单人局占比在 40% 上下活动」说的是**均值**—— 均值高于线并不等于
  // 每一代都高于线。抬到 0.33 是在抬高「连续三代都在线上」这个门槛，
  // 让局面多稳定一阵。
  //
  // ⚠ 死之执那条线**保持 0.05 没动** —— 两条线依然不对称，两边难度依然
  // 差得远。这条 0.33 与 0.05 一样**未经跑分标定**，现在由「游戏」面板调整。
  lifeWinRatio: 0.33,
  deathWinRatio: 0.05,
  lifeStreak: 3,
  deathStreak: 3,
 ...
 * - **`lifeWinRatio` 取 0.5，不是 8×8 那个 0.33** —— 这是被实测逼出来的：
 *   0.33 在 4×4 上只等于「≥ 6 格」，而预设开局 beacon / toad 本来就是 6 格
 *   （37.5%），**一开局就已经越过胜负线**，三代后生之执直接获胜
 *   （`presets.test.ts` 那条「每个开局至少能撑 20 代」当场变红）。
 *
 *   根因是**小棋盘的分辨率粗**：4×4 上一格就是 6.25%，0.33 落不到一个像样的
 *   格数上。所以这一档的阈值必须**更高**，而不是跟着大棋盘一起降
 * - `deathWinRatio` 保持 0.05，但它在 4×4 上的含义是「清空棋盘」：
 *   单格就占 6.25%，0.05 与 0 在这块棋盘上是同一条线。**这一档本就标着
 *   `experimental: true`、不参与跨尺寸比较**
