...
const OPENINGS_16: readonly Opening[] = [
  opening({
    ...
    note:
      ...
      "死格 199 个，生之执落点极多；但敲掉方块会引发连锁，" +
      "实测 57 → 65 → 55 → 46 → 43 … 20 代后落到 28 格（不是一代就崩，是先涨后落）。" +
      "⚠ 阵列表面的「静止」是假的 —— 单独看方块阵（不含那 3 盏灯），死之执 48 个落点里" +
      "只有 4 个会精确长回原局面，另外 44 个都会改变局面：敲掉一格之后，" +
      "方块之间那条 1 格空隙会凑够 3 个邻居一起诞生，相邻两块并成一条 5 格横条。" +
      "所以它并不是死局，第 1 代也不会 repeatBlocked。",
  }),
  opening({
    id: "dense-random",
    nameZh: "高密度随机",
